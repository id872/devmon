...
class SqlQuery
{
    public static function updatePowerDayStats($dbCon)
    {
        if (!$dbCon)
            return;
        $query = 
            "REPLACE INTO power_day_stats(user_id, device_id, day_production, kwh)
            SELECT user_id, device_id, date_format(readout_time, '%Y-%m-%d'), ROUND(SUM(ac_power) / 120 / 1000, 2) FROM view_power_logs 
