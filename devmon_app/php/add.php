...
function decodeAES($key, $encData)
{
    ...
    {
        return zlib_decode(openssl_decrypt($enc, 'AES-256-CFB8',
            $pass, OPENSSL_RAW_DATA|OPENSSL_ZERO_PADDING, $iv));
    }
    else
    {
        return NULL;
...
function decodeToJson($key, $encData)
{
    ...
    if (is_array($jsonData) &&
        array_key_exists("username", $jsonData) &&
        array_key_exists("user_password", $jsonData))
    {
        return $jsonData;
    }
    return NULL;
    ...
    array_key_exists("data", $_POST))
{
    ...
    if (!empty(array_values($auth)))
    {
        ...
        if ($json !== NULL &&
            checkUserPassword($json, $auth["user_password_hash"]))
        {
            $sql->addData($json);
        }
    }
